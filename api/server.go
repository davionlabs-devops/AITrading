...
func corsMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// 如果没有Origin头或不在允许列表中，使用默认值
		if requestOrigin == "" || !originAllowed {
			if allowedOrigin != "*" {
				allowedOrigin = origins[0] // 使用第一个允许的源
			}
		}
...
type UpdateExchangeConfigRequest struct {
	Exchanges map[string]struct {
		...
		ApexAPIKey            string `json:"apex_api_key"`
		ApexAPISecret         string `json:"apex_api_secret"`
		ApexPassphrase        string `json:"apex_passphrase"`
		ApexOmniKeySeed       string `json:"apex_omni_key_seed"`
	} `json:"exchanges"`
}
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	// 校验杠杆值
	if req.BTCETHLeverage < 1 || req.BTCETHLeverage > 100 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "BTC/ETH杠杆必须在1-100倍之间"})
		return
	}
	if req.AltcoinLeverage < 1 || req.AltcoinLeverage > 100 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "山寨币杠杆必须在1-100倍之间"})
		return
	}
	...
	if exchangeCfg == nil {
		...
		case "apex":
			tempTrader, createErr = trader.NewApexTrader(
				exchangeCfg.ApexAccountID,
				exchangeCfg.ApexAPIKey,
				exchangeCfg.ApexAPISecret,
				exchangeCfg.ApexPassphrase,
				exchangeCfg.ApexOmniKeySeed,
				false, // testnet
			)
		default:
...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	// 设置系统提示词模板，如果请求中有值则使用，否则保持原值
	systemPromptTemplate := existingTrader.SystemPromptTemplate
	if req.SystemPromptTemplate != "" {
		systemPromptTemplate = req.SystemPromptTemplate
	} else if systemPromptTemplate == "" {
		systemPromptTemplate = "default" // 如果原值为空，使用默认值
	}
