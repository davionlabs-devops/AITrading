...
func (d *Database) createTables() error {
	queries := []string{
		...
		`CREATE TABLE IF NOT EXISTS exchanges (
			...
			-- APEX 特定字段
			apex_account_id TEXT DEFAULT '',
			apex_api_key TEXT DEFAULT '',
			apex_api_secret TEXT DEFAULT '',
			apex_passphrase TEXT DEFAULT '',
			apex_omni_key_seed TEXT DEFAULT '',
			created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
			updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
			FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
		)`,
	...
	alterQueries := []string{
		...
		`ALTER TABLE exchanges ADD COLUMN apex_account_id TEXT DEFAULT ''`,
		`ALTER TABLE exchanges ADD COLUMN apex_api_key TEXT DEFAULT ''`,
		`ALTER TABLE exchanges ADD COLUMN apex_api_secret TEXT DEFAULT ''`,
		`ALTER TABLE exchanges ADD COLUMN apex_passphrase TEXT DEFAULT ''`,
		`ALTER TABLE exchanges ADD COLUMN apex_omni_key_seed TEXT DEFAULT ''`,
...
type ExchangeConfig struct {
	...
	// APEX 特定字段
	ApexAccountID   string    `json:"apexAccountId"`
	ApexAPIKey      string    `json:"apexApiKey"`
	ApexAPISecret   string    `json:"apexApiSecret"`
	ApexPassphrase  string    `json:"apexPassphrase"`
	ApexOmniKeySeed string    `json:"apexOmniKeySeed"`
	CreatedAt       time.Time `json:"created_at"`
	UpdatedAt       time.Time `json:"updated_at"`
}
...
func (d *Database) GetExchanges(userID string) ([]*ExchangeConfig, error) {
	rows, err := d.db.Query(`
		SELECT id, user_id, name, type, enabled, api_key, secret_key, testnet, 
		       ...
		       COALESCE(apex_account_id, '') as apex_account_id,
		       COALESCE(apex_api_key, '') as apex_api_key,
		       COALESCE(apex_api_secret, '') as apex_api_secret,
		       COALESCE(apex_passphrase, '') as apex_passphrase,
		       COALESCE(apex_omni_key_seed, '') as apex_omni_key_seed,
		       created_at, updated_at 
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&exchange.ApexAccountID, &exchange.ApexAPIKey,
			&exchange.ApexAPISecret, &exchange.ApexPassphrase,
			&exchange.ApexOmniKeySeed,
...
func (d *Database) UpdateExchange(userID, id string, enabled bool, apiKey, secretKey string, testnet bool, hyperliquidWalletAddr, asterUser, asterSigner, asterPrivateKey, apexAccountID, apexAPIKey, apexAPISecret, apexPassphrase, apexOmniKeySeed string) error {
	...
	result, err := d.db.Exec(`
		UPDATE exchanges SET enabled = ?, api_key = ?, secret_key = ?, testnet = ?, 
		       ...
		       apex_account_id = ?, apex_api_key = ?, apex_api_secret = ?, apex_passphrase = ?, apex_omni_key_seed = ?,
		       updated_at = datetime('now')
		WHERE id = ? AND user_id = ?
	`, enabled, apiKey, secretKey, testnet, hyperliquidWalletAddr, asterUser, asterSigner, asterPrivateKey,
		apexAccountID, apexAPIKey, apexAPISecret, apexPassphrase, apexOmniKeySeed, id, userID)
	if err != nil {
	...
	if rowsAffected == 0 {
		...
		`, id, userID, name, typ, enabled, apiKey, secretKey, testnet, hyperliquidWalletAddr, asterUser, asterSigner, asterPrivateKey,
			apexAccountID, apexAPIKey, apexAPISecret, apexPassphrase, apexOmniKeySeed)
