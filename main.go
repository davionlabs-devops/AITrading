...
func main() {
	...
	if jwtSecret == "" {
		...
		if jwtSecret == "" {
			...
			log.Printf("⚠️  JWT_SECRET未设置，已自动生成随机密钥")
			log.Printf("💡 建议设置环境变量: export JWT_SECRET='%s'", jwtSecret)
		} else {
			log.Printf("⚠️  从config.json读取JWT密钥，建议使用环境变量JWT_SECRET")
		}
	} else {
		log.Printf("✓ 从环境变量读取JWT密钥")
	}
