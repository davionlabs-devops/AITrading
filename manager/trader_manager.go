...
func (tm *TraderManager) addTraderFromDB(traderCfg *config.TraderRecord, aiModelCfg *config.AIModelConfig, exchangeCfg *config.ExchangeConfig, coinPoolURL, oiTopURL string, maxDailyLoss, maxDrawdown float64, stopTradingMinutes int, defaultCoins []string, database *config.Database, userID string) error {
	...
	if exchangeCfg.ID == "binance" {
		...
		traderConfig.ApexAccountID = exchangeCfg.ApexAccountID
		traderConfig.ApexAPIKey = exchangeCfg.ApexAPIKey
		traderConfig.ApexAPISecret = exchangeCfg.ApexAPISecret
		traderConfig.ApexPassphrase = exchangeCfg.ApexPassphrase
		traderConfig.ApexOmniKeySeed = exchangeCfg.ApexOmniKeySeed
	}
	...
	log.Printf("✓ Trader '%s' (%s + %s) 已加载到内存 [模板: %s]", traderCfg.Name, aiModelCfg.Provider, exchangeCfg.ID, systemPromptTemplate)
	return nil
}
...
func (tm *TraderManager) LoadUserTraders(database *config.Database, userID string) error {
	...
	for _, traderCfg := range traders {
		...
		if existingTrader, exists := tm.traders[traderCfg.ID]; exists {
			// 如果交易员已存在，更新其配置（特别是系统提示词模板）
			log.Printf("🔄 交易员 %s 已存在，更新配置...", traderCfg.Name)

			// 更新系统提示词模板
			if existingTrader.GetSystemPromptTemplate() != traderCfg.SystemPromptTemplate {
				log.Printf("  📋 更新系统提示词模板: %s -> %s",
					existingTrader.GetSystemPromptTemplate(), traderCfg.SystemPromptTemplate)
				existingTrader.SetSystemPromptTemplate(traderCfg.SystemPromptTemplate)
			}

			// 更新自定义提示词
			existingTrader.SetCustomPrompt(traderCfg.CustomPrompt)
			existingTrader.SetOverrideBasePrompt(traderCfg.OverrideBasePrompt)

			log.Printf("✓ 交易员 %s 配置已更新", traderCfg.Name)
			continue
		}
...
func (tm *TraderManager) loadSingleTrader(traderCfg *config.TraderRecord, aiModelCfg *config.AIModelConfig, exchangeCfg *config.ExchangeConfig, coinPoolURL, oiTopURL string, maxDailyLoss, maxDrawdown float64, stopTradingMinutes int, defaultCoins []string, database *config.Database, userID string) error {
	...
	log.Printf("✓ Trader '%s' (%s + %s) 已为用户加载到内存 [模板: %s]", traderCfg.Name, aiModelCfg.Provider, exchangeCfg.ID, systemPromptTemplate)
	return nil
}
