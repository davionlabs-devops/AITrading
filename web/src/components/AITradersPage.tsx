...
export function AITradersPage({ onTraderSelect }: AITradersPageProps) {
  ...
  // 只显示已配置的交易所（有API密钥或必要配置字段的）
  const configuredExchanges = (allExchanges || []).filter((e) => {
    // 检查是否有必要的配置字段
    if (e.id === 'binance' || e.id === 'okx') {
      return !!(e.apiKey && e.secretKey)
    } else if (e.id === 'hyperliquid') {
      return !!e.apiKey
    } else if (e.id === 'aster') {
      return !!(e.asterUser && e.asterSigner && e.asterPrivateKey)
    } else if (e.id === 'apex') {
      return !!(e.apexApiKey && e.apexApiSecret && e.apexPassphrase && e.apexAccountId)
    } else {
      // 其他CEX交易所
      return !!(e.apiKey && e.secretKey)
    }
  ...
  const handleDeleteExchangeConfig = async (exchangeId: string) => {
    ...
    try {
      ...
      const exchangeToDelete = updatedExchanges.find((e) => e.id === exchangeId)
      if (exchangeToDelete) {
        if (exchangeToDelete.id === 'apex') {
          request.exchanges[exchangeToDelete.id] = {
            enabled: false,
            apex_account_id: '',
            apex_api_key: '',
            apex_api_secret: '',
            apex_passphrase: '',
            apex_omni_key_seed: '',
            testnet: exchangeToDelete.testnet || false,
          }
        } else if (exchangeToDelete.id === 'aster') {
          request.exchanges[exchangeToDelete.id] = {
            enabled: false,
            aster_user: '',
            aster_signer: '',
        ...
        } else if (exchangeToDelete.id === 'hyperliquid') {
          request.exchanges[exchangeToDelete.id] = {
            enabled: false,
            api_key: '',
            testnet: exchangeToDelete.testnet || false,
  ...
  const handleSaveExchangeConfig = async (
    ...
    apexApiKey?: string,
    apexApiSecret?: string,
    apexPassphrase?: string,
    apexOmniKeySeed?: string
  ) => {
    try {
        ...
        exchanges: Object.fromEntries(
          updatedExchanges.map((exchange) => [
            exchange.id,
            {
              ...
              apex_account_id: exchange.apexAccountId || '',
              apex_api_key: exchange.apexApiKey || '',
              apex_api_secret: exchange.apexApiSecret || '',
              apex_passphrase: exchange.apexPassphrase || '',
              apex_omni_key_seed: exchange.apexOmniKeySeed || '',
            },
          ])
        ),
      }
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div>
            ...
            >
              ...
              >
                {traders?.filter((t) => t && t.trader_id && t.trader_name && t.trader_name.trim() !== '').length || 0} {t('active', language)}
              </span>
            </h1>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs" style={{ color: '#848E9C' }}>
                        ...
                        {inUse
                          ? t('inUse', language)
                          : (() => {
                              // 检查是否真正配置了（有API密钥或必要的配置字段）
                              let hasConfig = false
                              if (exchange.id === 'binance' || exchange.id === 'okx') {
                                hasConfig = !!(exchange.apiKey && exchange.secretKey)
                              } else if (exchange.id === 'hyperliquid') {
                                hasConfig = !!exchange.apiKey
                              } else if (exchange.id === 'aster') {
                                hasConfig = !!(exchange.asterUser && exchange.asterSigner && exchange.asterPrivateKey)
                              } else if (exchange.id === 'apex') {
                                hasConfig = !!(exchange.apexApiKey && exchange.apexApiSecret && exchange.apexPassphrase && exchange.apexAccountId)
                              } else {
                                hasConfig = !!(exchange.apiKey && exchange.secretKey)
                              }
                              
                              if (exchange.enabled && hasConfig) {
                                return t('enabled', language)
                              } else if (hasConfig) {
                                return t('configured', language)
                              } else {
                                return t('notConfigured', language)
                              }
                            })()}
                      </div>
                    </div>
                  ...
                  <div
                    className={`w-2.5 h-2.5 md:w-3 md:h-3 rounded-full flex-shrink-0 ${
                      (() => {
                        ...
                        return exchange.enabled && hasConfig ? 'bg-green-400' : 'bg-gray-500'
                      })()
                    }`}
                  />
                </div>
      ...
      <div className="binance-card p-4 md:p-6">
        ...
        {traders && traders.length > 0 ? (
          <div className="space-y-3 md:space-y-4">
            {traders
              .filter((trader) => trader && trader.trader_id && trader.trader_name && trader.trader_name.trim() !== '')
              .map((trader) => (
              <div
              ...
              >
                <div className="flex items-center gap-3 md:gap-4">
                  <div
                    ...
                    style={{
                      background: trader.ai_model && trader.ai_model.includes('deepseek')
                        ? '#60a5fa'
                        : '#c084fc',
                      color: '#fff',
                    }}
                  ...
                  <div className="min-w-0">
                    <div
                      ...
                      style={{ color: '#EAECEF' }}
                    >
                      {trader.trader_name || '未命名交易员'}
                    </div>
                    <div
                      className="text-xs md:text-sm truncate"
                      style={{
                        color: trader.ai_model && trader.ai_model.includes('deepseek')
                          ? '#60a5fa'
                          : '#c084fc',
                      }}
                    >
                      {trader.ai_model ? getModelDisplayName(
                        trader.ai_model.split('_').pop() || trader.ai_model
                      ) : '未知模型'}{' '}
                      Model • {trader.exchange_id?.toUpperCase() || '未知交易所'}
                    </div>
                  </div>
                </div>
...
function ExchangeConfigModal({
  ...
  // APEX 特定字段
  const [apexAccountId, setApexAccountId] = useState('')
  const [apexApiKey, setApexApiKey] = useState('')
  const [apexApiSecret, setApexApiSecret] = useState('')
  const [apexPassphrase, setApexPassphrase] = useState('')
  const [apexOmniKeySeed, setApexOmniKeySeed] = useState('')
  ...
  // 当editingExchangeId变化时，更新selectedExchangeId
  useEffect(() => {
    if (editingExchangeId) {
      setSelectedExchangeId(editingExchangeId)
    }
  }, [editingExchangeId])
  ...
  useEffect(() => {
    if (editingExchangeId && selectedExchange) {
      ...
      // APEX 字段
      setApexAccountId(selectedExchange.apexAccountId || '')
      setApexApiKey(selectedExchange.apexApiKey || '')
      setApexApiSecret('') // Don't load existing secret for security
      setApexPassphrase('') // Don't load existing passphrase for security
      setApexOmniKeySeed('') // Don't load existing seed for security
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedExchange?.id === 'binance') {
      ...
      if (!apexAccountId.trim() || !apexApiKey.trim() || !apexApiSecret.trim() || !apexPassphrase.trim())
        return
      await onSave(
        selectedExchangeId,
        apexApiKey.trim(),
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      >
        {/* 顶部固定区域 */}
        <div className="flex items-center justify-between p-6 pb-4 flex-shrink-0 border-b" style={{ borderColor: '#2B3139' }}>
          <h3 className="text-xl font-bold" style={{ color: '#EAECEF' }}>
            {editingExchangeId
        ...
        >
          ...
          `}</style>
          <form id="exchange-config-form" onSubmit={handleSubmit} className="space-y-4">
          {!editingExchangeId && (
            <div>
              <label
                className="block text-sm font-semibold mb-2"
                ...
                <>
                  <div>
                    ...
                    <input
                      type="text"
                      value={apexAccountId}
                      onChange={(e) => setApexAccountId(e.target.value)}
                      placeholder="输入APEX Account ID"
                      className="w-full px-3 py-2 rounded"
                      style={{
                        background: '#0B0E11',
                        border: '1px solid #2B3139',
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={apexApiSecret}
                      onChange={(e) => setApexApiSecret(e.target.value)}
                      placeholder="输入APEX API Secret"
                      className="w-full px-3 py-2 rounded"
                      style={{
                        background: '#0B0E11',
                        border: '1px solid #2B3139',
                  ...
                  <div>
                    ...
                    <input
              type="text"
                      value={apexOmniKeySeed}
                      onChange={(e) => setApexOmniKeySeed(e.target.value)}
                      placeholder="输入APEX Omni Key Seed (可选)"
                      className="w-full px-3 py-2 rounded"
                      style={{
                        background: '#0B0E11',
                        border: '1px solid #2B3139',
        ...
        {/* 底部固定区域 */}
        <div className="p-6 pt-4 flex-shrink-0 border-t flex gap-3" style={{ borderColor: '#2B3139' }}>
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2 rounded text-sm font-semibold"
